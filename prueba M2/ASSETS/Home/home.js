document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('cancel-transaction')) {
         ...
         // Recalcular el total de la altura después de un pequeño retraso para que se aplique después de que se elimine la transacción
    setTimeout(function() {
        var newTotalHeight = transactionList.scrollHeight;
        transactionList.style.paddingBottom = newTotalHeight + 'px';
    }, 100);
});
